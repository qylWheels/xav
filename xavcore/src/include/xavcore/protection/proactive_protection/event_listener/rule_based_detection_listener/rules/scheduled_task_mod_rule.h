...
struct ScheduledTaskModRuleWarningInfo : public IRuleWarningInfo {
    ScheduledTaskModRuleWarningInfo(Process process, std::string path)
    ...
    virtual std::shared_ptr<IRuleWarningInfo> clone() const override {
        return std::make_shared<ScheduledTaskModRuleWarningInfo>(*this);
    }
...
class ScheduledTaskModRule : public IRuleBasedDetectionListenerRule {
public:
    ScheduledTaskModRule();
    ~ScheduledTaskModRule();
    ScheduledTaskModRule(const ScheduledTaskModRule&) = delete;
    ScheduledTaskModRule& operator=(const ScheduledTaskModRule&) = delete;
    ScheduledTaskModRule(ScheduledTaskModRule&&) = delete;
    ScheduledTaskModRule& operator=(ScheduledTaskModRule&&) = delete;
